...
ignore_broadcast_ssid=0" > hostapd.conf
  ...
  x-terminal-emulator -e sudo dnsmasq -C dnsmasq.conf -d --no-resolv --no-hosts --dns-loop-detect --quiet-ra --quiet-dhcp6 --quiet-dhcp
  ...
  #start hostapd
  x-terminal-emulator -e sudo hostapd hostapd.conf
}
